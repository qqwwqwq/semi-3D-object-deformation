...
def get_KNN_points(center_point, xyz, k):
    new = np.tile(center_point, (len(xyz), 1))
    delta = xyz - new
...
pcd =  o3d.io.read_point_cloud(rootpath+'/11.13/ts.ply')
pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamKNN(knn=30))
normal1 = np.asarray(pcd.normals)
xyz1=np.array(pcd.points)


k = 50
xyz, normal = xyz1, normal1
KH = np.zeros((len(xyz), 1))
KG = np.zeros((len(xyz), 1))
K1 = np.zeros((len(xyz), 1))
K2 = np.zeros((len(xyz), 1))
print(xyz.shape)
